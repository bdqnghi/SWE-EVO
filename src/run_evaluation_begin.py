...
thaiminhpv/sweworld-numpy_v2.1.3:latest
        ...
        # else:
        #     logger.info(f"{APPLY_PATCH_PASS}:\n{val.output.decode('utf-8')}")
        _output = container.exec_run("git show", workdir=repo_directory).output.decode("utf-8").strip()
        logger.info(f"Git show:\n{_output}")

        _output = container.exec_run("git revert HEAD", workdir=repo_directory).output.decode("utf-8").strip()
        logger.info(f"Git revert HEAD:\n{_output}")
